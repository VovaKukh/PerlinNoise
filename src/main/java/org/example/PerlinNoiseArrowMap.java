...
public class PerlinNoiseArrowMap extends Application {
    ...
    public void start(Stage stage) {
        ...
        drawCirclesAndArrows(gc, width, height, 0);
    ...
    private void drawCirclesAndArrows(GraphicsContext gc, int width, int height, double z) {
        int circleRadius = 2;
        int spacing = 20;
        int arrowLength = 13;
        double scale = 0.003;
        ...
        for (int x = 0; x < numCirclesX; x++) {
            for (int y = 0; y < numCirclesY; y++) {
                ...
                // Use Perlin noise to determine the direction of the arrow
                double noiseValue = PerlinNoise.noise(centerX * scale, centerY * scale, z);
                System.out.println(noiseValue);
                // Map the noise value to an angle. Assuming noiseValue is -0.5 to 0.5, map this to 0 to 2π radians
                double angle = (noiseValue + 0.5) * 2 * Math.PI;

                // Draw an arrow from each circle in the direction determined by Perlin noise
                drawArrow(gc, centerX, centerY, arrowLength, angle);
            }
        }
    }

    private void drawArrow(GraphicsContext gc, int startX, int startY, int length, double angle) {
